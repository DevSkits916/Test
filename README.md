...
Environment variables control runtime behavior. Copy `.env.example` to `.env` to start with sensible defaults and adjust as needed:
...
| `CONFIG_PATH` | `config/sources.json` | Override path to adapter configuration file. |
| `MIN_LEN` | `5` | Minimum length for extracted codes. |
| `MAX_LEN` | `8` | Maximum length for extracted codes. |
